...
// based on https://github.com/words/double-metaphone/blob/main/index.js
...
export function doubleMetaphone(value: string): [string, string] {
  ...
  while (index < length) {
    ...
    const nextnext = characters[index + 2] as string
    /** @type {string} */
    ...
    switch (characters[index]) {
      ...
      case 'C':
        ...
        if (
          ...
          && next === 'H'
          && nextnext !== 'I'
          && !vowels.test(characters[index - 2] as string)
          && (nextnext !== 'E'
      ...
      case 'G':
        ...
        subvalue === 'B' || subvalue === 'H')
          ...
          if (
            index > 2
            && previous === 'U'
            && gForF.test(characters[index - 3] as string)
          ) {
            primary += 'F'
            secondary += 'F'
          }
        ...
        if (next === 'N') {
          if (index === 1 && vowels.test(characters[0] as string) && !isSlavoGermanic) {
            primary += 'KN'
            secondary += 'N'
            // Not like `Cagney`.
          }
      ...
      case 'S':
        ...
        if (next === 'C') {
          ...
          if (nextnext === 'H') {
            ...
            if (
              index === 0
              && !vowels.test(characters[3] as string)
              && characters[3] !== 'W'
            ) {
              primary += 'X'
              secondary += 'S'

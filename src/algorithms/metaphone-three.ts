 ...
 *
 * Metaphone 3 is designed to return an *approximate* phonetic key (and an
 * alternate
 ...
 * has
 * proven to be useful in compensating for variations in pronunciation, as well
 * as
 * misheard pronunciations. For example, although americans are not usually
 * aware of it,
 * the letter 's' is normally pronounced 'z' at the end of words such as
 * "sounds".<br>
 * <br>
 *
 * The 'approximate' aspect of the encoding is implemented according to the
 * following rules:<br>
 * <br>
 *
 * (1) All vowels are encoded to the same value - 'A'. If the parameter
 * encodeVowels
 * is set to false, only *initial* vowels will be encoded at all. If
 * encodeVowels is set
 * to true, 'A' will be encoded at all places in the word that any vowels are
 * normally
 * pronounced. 'W' as well as 'Y' are treated as vowels. Although there are
 * differences in
 * the pronunciation of 'W' and 'Y' in different circumstances that lead to
 * their being
 * classified as vowels under some circumstances and as consonants in others,
 * for the purposes
 * of the 'fuzziness' component of the Soundex and Metaphone family of
 * algorithms they will
 * be always be treated here as vowels.<br>
 * <br>
 *
 * (2) Voiced and un-voiced consonant pairs are mapped to the same encoded
 * value. This
 * means that:<br>
 * 'D' and 'T' -> 'T'<br>
 * 'B' and 'P' -> 'P'<br>
 ...
 * <br>
 *
 * - In addition to the above voiced/unvoiced rules, 'CH' and 'SH' -> 'X', where
 * 'X'
 ...
 * symbol). (Although
 * Americans are not usually aware of it, "TH" is pronounced in a voiced (e.g.
 * "that") as
 * well as an unvoiced (e.g. "theater") form, which are naturally mapped to the
 * same encoding.)<br>
 * <br>
 *
 * The encodings in this version of Metaphone 3 are according to pronunciations
 * common in the
 * United States. This means that they will be inaccurate for consonant
 * pronunciations that
 * are different in the United Kingdom, for example "tube" -> "CHOOBE" -> XAP
 * rather than american TAP.<br>
 * <br>
 *
 * Metaphone 3 was preceded by by Soundex, patented in 1919, and Metaphone and
 * Double Metaphone,
 * developed by Lawrence Philips. All of these algorithms resulted in a
 * significant number of
 * incorrect encodings. Metaphone3 was tested against a database of about 100
 * thousand English words,
 * names common in the United States, and non-English words found in
 * publications in the United States,
 * with an emphasis on words that are commonly mispronounced, prepared by the
 * Moby Words website,
 * but with the Moby Words 'phonetic' encodings algorithmically mapped to Double
 * Metaphone encodings.
 * Metaphone3 increases the accuracy of encoding of english words, common names,
 * and non-English
 * words found in american publications from the 89% for Double Metaphone, to
 * over 98%.<br>
 * <br>
 *
 * DISCLAIMER:
 * Anthropomorphic Software LLC claims only that Metaphone 3 will return correct
 * encodings,
 * within the 'fuzzy' definition of correct as above, for a very high percentage
 * of correctly
 * spelled English and commonly recognized non-English words. Anthropomorphic
 * Software LLC
 * warns the user that a number of words remain incorrectly encoded, that
 * misspellings may not
 * be encoded 'properly', and that people often have differing ideas about the
 * pronunciation
 * of a word. Therefore, Metaphone 3 is not guaranteed to return correct results
 * every time, and
 * so a desired target word may very well be missed. Creators of commercial
 * products should
 * keep in mind that systems like Metaphone 3 produce a 'best guess' result, and
 * should
 ...
 *
 * Metaphone 3 is designed to return an <i>approximate</i> phonetic key
 * (and an alternate
 * approximate phonetic key when appropriate) that should be the same
 * for English
 * words, and most names familiar in the United States, that are
 * pronounced "similarly".
 * The key value is <i>not</i> intended to be an exact phonetic, or even
 * phonemic,
 * representation of the word. This is because a certain degree of
 * 'fuzziness' has
 * proven to be useful in compensating for variations in pronunciation,
 * as well as
 * misheard pronunciations. For example, although americans are not
 * usually aware of it,
 * the letter 's' is normally pronounced 'z' at the end of words such as
 * "sounds".<br>
 * <br>
 *
 * The 'approximate' aspect of the encoding is implemented according to
 ...
 * is set to false, only *initial* vowels will be encoded at all. If
 * encodeVowels is set
 * to true, 'A' will be encoded at all places in the word that any
 * vowels are normally
 * pronounced. 'W' as well as 'Y' are treated as vowels. Although there
 * are differences in
 * the pronunciation of 'W' and 'Y' in different circumstances that lead
 * to their being
 * classified as vowels under some circumstances and as consonants in
 * others, for the purposes
 * of the 'fuzziness' component of the Soundex and Metaphone family of
 * algorithms they will
 * be always be treated here as vowels.<br>
 * <br>
 ...
 * <br>
 *
 * - In addition to the above voiced/unvoiced rules, 'CH' and 'SH' ->
 * 'X', where 'X'
 * represents the "-SH-" and "-CH-" sounds in Metaphone 3 encoding.<br>
 * <br>
 *
 * - Also, the sound that is spelled as "TH" in English is encoded to
 * '0' (zero symbol). (Although
 * americans are not usually aware of it, "TH" is pronounced in a voiced
 * (e.g. "that") as
 * well as an unvoiced (e.g. "theater") form, which are naturally mapped
 * to the same encoding.)<br>
 * <br>
 *
 * In the "Exact" encoding, voiced/unvoiced pairs are <i>not</i> mapped
 * to the same encoding, except
 * for the voiced and unvoiced versions of 'TH', sounds such as 'CH' and
 * 'SH', and for 'S' and 'Z',
 * so that the words whose metaph keys match will in fact be closer in
 * pronunciation that with the
 * more approximate setting. Keep in mind that encoding settings for
 * search strings should always
 * be exactly the same as the encoding settings of the stored metaph
 * keys in your database!
 * Because of the considerably increased accuracy of Metaphone3, it is
 * now possible to use this
 * setting and have a very good chance of getting a correct encoding.
 * <br>
 * <br>
 * In the Encode Vowels encoding, all non-initial vowels and diphthongs
 * will be encoded to
 * 'A', and there will only be one such vowel encoding character between
 * any two consonants.
 * It turns out that there are some surprising wrinkles to encoding
 * non-initial vowels in
 * practice, pre-eminently in inversions between spelling and
 * pronunciation such as e.g.
 * "wrinkle" => 'RANKAL', where the last two sounds are inverted when
 * spelled.
 * <br>
 * <br>
 * The encodings in this version of Metaphone 3 are according to
 * pronunciations common in the
 * United States. This means that they will be inaccurate for consonant
 * pronunciations that
 * are different in the United Kingdom, for example "tube" -> "CHOOBE"
 * -> XAP rather than american TAP.
 * <br>
 * <br>
 *
 * @class
 */
 class Metaphone3 {
   ...
   *
   * @param {string} incomingCharString pointer to EXTERNALLY ALLOCATED char string of
  ...
  public MetaphAdd$java_lang_String$java_lang_String(
    main: string,
    alt: string,
  ) {
    if (
   ...
   * @param {string} alt  alternative encoding character to be added to encoded alternative
   * key string
   */
  ...
  /**
   * Adds an encoding character to the encoded key value string - Exact/Approx
   * version
   *
   * @param {string} mainExact primary encoding character to be added to encoded key string
   * if
   * m_encodeExact is set
   *
   * @param {string} altExact  alternative encoding character to be added to encoded
   * alternative
   * key string if m_encodeExact is set
   *
   * @param {string} main      primary encoding character to be added to encoded key string
   *
   * @param {string} alt       alternative encoding character to be added to encoded
   * alternative key string
   */
  public MetaphAddExactApprox(
    mainExact?: any,
    altExact?: any,
    main?: any,
    alt?: any,
   ...
   *
   * @return {number} short integer representing the length of allocated storage for the
   * key.
   */
   ...
   * vowel diphthong, etc.), only one 'A' will be encoded before the next
   * consonant or the
   * end of the word.
   *
  ...
  /**
   * Retrieves setting determining whether or not non-initial vowels will be
   * encoded.
   *
   * @return {boolean} true if the Metaphone3 object has been set to encode non-initial
   * vowels, false if not.
   */
   ...
   * at the end of many words as 'Z', nor does it include "CH" vs. "SH". It does
   * cause
   * a distinction to be made between 'B' and 'P', 'D' and 'T', 'G' and 'K', and
   * 'V'
   * and 'F'.
   *
   * @param {boolean} inEncodeExact consonants to be encoded "exactly" if true, not if
   * false.
   */
  ...
  /**
   * Retrieves setting determining whether or not consonants will be encoded
   * "exactly".
   *
   * @return {boolean} true if the Metaphone3 object has been set to encode "exactly", false
   * if not.
   */
   ...
   * will not be encoded.
   *
   * @param {number} at position, in string to be encoded, of character to start skipping
   * from
   *
  ...
  /**
   * Advanced counter m_current so that it indexes the next character to be
   * encoded
   *
   * @param {number} ifNotEncodeVowels number of characters to advance if not encoding
   * internal vowels
   * @param {number} ifEncodeVowels    number of characters to advance if encoding internal
   * vowels
   */
